...
import { ArrowLeft, Pill, Beaker, Save } from 'lucide-react';
  ...
  return (
    <div className="max-w-lg mx-auto pb-20">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-xl font-bold text-slate-900">Editar Admissão</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 bg-white p-6 rounded-xl shadow-sm border border-slate-200">
        ...
        <button
          ...
          className="w-full bg-medical-600 text-white font-bold py-4 rounded-xl shadow hover:bg-medical-700 transition-colors disabled:opacity-50 mt-6 flex items-center justify-center gap-2"
        >
          {isSubmitting ? 'Salvando...' : <><Save className="w-5 h-5" /> Atualizar Dados</>}
        </button>
      </form>
    </div>
  );
