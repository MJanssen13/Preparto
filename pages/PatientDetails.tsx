...
import { ArrowLeft, Plus, Droplets, Thermometer, Activity, Pill, LogOut, Clock, BedDouble, AlertCircle, CircleDot, Edit2, Beaker, Hammer, Wind, Waves } from 'lucide-react';
...
const PatientDetails: React.FC = () => {
  ...
  const handleDischarge = async () => {
    ...
    if (confirm('Deseja realmente dar alta/encerrar o atendimento para esta paciente?')) {
      await patientService.updatePatient(id, { status: PatientStatus.DISCHARGED });
      navigate('/');
    }
  };
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      <div className="flex items-center justify-between">
        ...
        {!isDischarged && (
          <button 
            ...
            className="flex flex-col items-center justify-center p-2 text-red-600 bg-red-50 hover:bg-red-100 rounded-lg border border-red-100 transition-colors"
          >
      ...
      {!isDischarged && pendingTasks.length > 0 && (
               ...
               <div className="grid grid-cols-2 gap-y-4 gap-x-2">
                  ...
                  <div className="col-span-2 grid grid-cols-3 gap-2 pb-3 border-b border-slate-100">
                     ...
                     <div className="flex flex-col">
                        ...
                        <span className="font-bold text-lg text-obs-500">
                     ...
                     <div className="flex flex-col">
                        ...
                        <div className="font-bold text-lg text-slate-700 leading-tight">
                            {obs.obstetric.dinamicaSummary ? (
                                <span className="text-xs font-bold block mt-1">{obs.obstetric.dinamicaSummary}</span>
                            ) : (
                                <span>{obs.obstetric.dinamicaFrequency !== undefined ? `${obs.obstetric.dinamicaFrequency}/10'` : '-'}</span>
                            )}
                        </div>
                     </div>
                  </div>
                  ...
                  {/* Expanded Obstetric Details (Toque) */}
                  {(obs.obstetric.cervixPosition || obs.obstetric.cervixConsistency || obs.obstetric.station !== undefined || obs.obstetric.effacement !== undefined) && (
                      <div className="col-span-2 bg-slate-50 p-2 rounded-lg border border-slate-100 text-xs text-slate-700 mt-1">
                          ...
                          <div className="grid grid-cols-2 gap-1">
                              {obs.obstetric.effacement !== undefined && <span>Apagamento: <b>{obs.obstetric.effacement}%</b></span>}
                              {obs.obstetric.station !== undefined && <span>De Lee: <b>{obs.obstetric.station > 0 ? '+' : ''}{obs.obstetric.station}</b></span>}
                              {obs.obstetric.cervixPosition && <span>Pos: <b>{obs.obstetric.cervixPosition}</b></span>}
                              {obs.obstetric.cervixConsistency && <span>Cons: <b>{obs.obstetric.cervixConsistency}</b></span>}
                          </div>
                      </div>
                  )}
